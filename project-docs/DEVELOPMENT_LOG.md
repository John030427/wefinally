...
## 2026-09-04 — 活动修改与餐饮询问协调修复

### 根因

- 短句“改为吃饭”未命中旧自然语言合约，模型未生成 patch 工具请求时会退回泛化答复，导致方案卡不变。
- “我想吃大二酸菜 / 问对方想不想吃酸菜鱼”同时包含活动、菜品说明和询问意图，旧路径容易当作普通聊天处理。
- “字段齐全”与“双方当前版本确认”曾在旧客户端方案卡中混淆；`meeting_ready` 不得代替服务端 `arranged` 门禁。

### 修复

- 明确活动变更使用确定性解析和 `create_date_application_patch` 预览；支持“改为/调整为/换成”，并取最后一个活动词。
- 餐饮表达生成“吃饭 + activity_detail”的待确认预览，确认后才沿用隐私安全的对方通知和当前版本校验。
- 保留用户填写的商圈/商场地点；只在 `activity_detail` 有值时归一化，公开变更维度去重，保持旧记录字段兼容。
- 方案卡和对方询问卡显示活动说明；模型暂时不可用时，明确活动修改仍有确定性预览兜底，重复餐饮修改返回可读错误而不是 `SERVER_ERROR`。
- 新增 agent chat 断言，未删除历史匹配/聊天记录，未部署本次代码。

### 交接与验证

- 交接文档：`project-docs/handoff/2026-09-04-activity-change-coordination-handoff.md`。
- `selfcheck:agent`、安全/报告/支付/会员/匹配/QA/微信身份基线、agent-graph build + 42 tests、release workflow contract、`git diff --check` 均通过。
- 真实双设备和 `LIVE_GRAPH_SMOKE` 仍需上传同一客户端测试版后人工验收。

---

## 2026-09-04 — 自由地点发布前复核修复

- 修复公开方案、邀请、偏好合并、重新计算及直接接受路径丢失地点安排方式和未定事项的问题。
- 客户端/模型提交的 `accepted_by` 不作为同意凭据；最终确认在数据库事务中检查当前版本的双方确认。直接接受以发送当前邀请和受邀者接受为凭据。
- 星巴克看电影允许发出邀请，但成约前必须明确是否先碰面；显式 `meet_first` 可保留会合地点和影院待商量事项。已有电影院时，“星巴克只是集合点”不覆盖电影院。
- 邀请卡取消提前宣称双方已确认；聊天方案卡增加约定事项。沿用现有 WeFinally 样式，不新增布局或视觉资源。
- 补充公开字段链路、伪造接受标记、同版双确认、旧版确认、冲突回滚、直接接受事务测试。
- 验证：8 组 server 基线、release-workflow-contract、agent-graph build + 42 tests 均通过。真实模型/双真机验收仍需部署后执行，不能由单测代替。
- 不修改数据库内容、密钥、运行时、权限或模型配置，不上传客户端。部署结果另记 `project-docs/releases/2026-09-04-flexible-location-review.md`。
