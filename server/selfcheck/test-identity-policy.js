...
assert.strictEqual(resolveTestIdentity(legacyQa).kind, 'real_user')
assert.strictEqual(isInternalQaAccount({ ...real, openid: 'test_spoofed_user' }), false)
...
assert.strictEqual(canEnterFormalCandidatePool(qa), false)
assert.strictEqual(canEnterFormalCandidatePool(synthetic), false)
assert.strictEqual(canEnterFormalCandidatePool({ ...real, profile_origin: 'real_user', is_test_fixture: 1 }), false)
assert.strictEqual(canEnterFormalCandidatePool({ ...real, profile_origin: 'real_user', fixture_owner_user_id: 10 }), false)
assert.strictEqual(canEnterFormalCandidatePool({ ...real, test_scope: 'matching' }), false)
assert.strictEqual(canEnterFormalCandidatePool({ ...real, is_match_effect_fixture: 1 }), false)
assert.strictEqual(canEnterFormalCandidatePool({ ...real, fixture_access_mode: 'public_test_pool' }), false)
assert.strictEqual(canEnterFormalCandidatePool({ ...real, formal_match_hidden: true }), false)
assert.strictEqual(canEnterFormalCandidatePool({ ...real, profile_origin: 'unexpected_origin' }), false)
assert.strictEqual(canEnterFormalCandidatePool(legacyQa), false)
