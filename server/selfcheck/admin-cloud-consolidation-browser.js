...
async function main() {
  ...
  try {
    ...
    await evaluate(`selectCloudConversation(303)`)
    await waitUntil(`document.querySelectorAll('.service-paired-grid .service-side').length === 2 && document.getElementById('content').textContent.includes('A侧：周六下午可以喝咖啡。') && document.getElementById('content').textContent.includes('B侧：周六下午也可以。')`, 'paired A/B date conversation')
    assert((await evaluate(`document.querySelector('.service-paired-shared').textContent`)).includes('公共协调进度'))
    await evaluate(`document.querySelector('button[onclick="selectPairedConversation(304)"]').click()`)
    await waitUntil(`document.querySelector('.service-conversation-title').textContent.includes('WF-S-000304') && document.querySelector('[data-side="B"] .service-side-badge')`, 'switch paired conversation side')
