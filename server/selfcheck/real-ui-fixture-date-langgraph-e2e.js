...
async function main() {
  ...
  assert.ok(first.patch_preview, 'API must return the persisted backend patch preview for the chat card')
  assert.strictEqual(first.patch_preview.id, first.pending_preview.patchId)
  assert.ok(first.patch_preview.preview && Array.isArray(first.patch_preview.preview.changed_fields))
