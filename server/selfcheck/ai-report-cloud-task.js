...
assert(handler.includes('await api.persistOptionalReportAudit(task, attemptId, result, retention, generatedAt, api)'))
assert(handler.includes('reports_json: JSON.stringify(databaseSafe(result.reports))'))
assert(handler.includes('report_expires_at: retention.report_expires_at'))
assert(handler.includes('input_expires_at: retention.input_expires_at'))
