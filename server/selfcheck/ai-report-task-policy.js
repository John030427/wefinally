...
assert.strictEqual(classifyError(new Error('MiniMax HTTP 429')).retryable, true)
assert.strictEqual(classifyError(new Error('socket hang up')).retryable, true)
assert.strictEqual(classifyError(new Error('getaddrinfo EAI_AGAIN api.minimaxi.com')).retryable, true)
