...
const { createReferralToken, verifyReferralToken, createReferralScene, verifyReferralScene, referralInput } = require('../../miniprogram/cloudfunctions/api/lib/partnerReferralPolicy')
...
const scene = createReferralScene(17, { secret, now: 1700000000000 })
assert(scene.length <= 32)
assert.strictEqual(verifyReferralScene(scene, { secret, now: 1700000000000 }).partnerId, 17)
assert.strictEqual(referralInput(scene, { secret, now: 1700000000000 }).partnerId, 17)
assert.strictEqual(referralInput(normalizePromoteCode(scene), { secret, now: 1700000000000 }).partnerId, 17)
assert.throws(() => verifyReferralScene(`${scene.slice(0, -1)}x`, { secret, now: 1700000000000 }), /归因标识无效/)
