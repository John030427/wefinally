...
**⚠️ 隐私展示规则（v2 修订，与现有产品对齐——违反即返工）：**
现有产品**全程不展示对方姓名/照片/职业/联系方式**，一律以「匹配对象 / 对方 / TA」指代（证据：`miniprogram/pages/match-list` 显示 `性别·年龄·城市`；`match-detail` 只显示 `年龄段/身高/学历/圈子/生育计划` 标签）。原型必须遵守：

- **可展示**：性别（他/她）、年龄、城市（到市，不到区）、身高段、学历、圈子名、生育计划、匹配类型、日期、状态词
- **禁止展示**：对方姓名（含姓氏 monogram）、照片、职业、住址、联系方式
- 对方指称：文字用「TA / 对方 / 本期介绍对象」，或用各方向自己的**编号体系**代称（FILE #025 / PASS WF-C08 / SIG-0847 / ISSUE NO.047 等）
- 用户本人的信息（陈晓雯）可以展示

**用户**：陈晓雯，29，深圳南山，产品经理。资料就绪度 92%。

**本次新匹配对象**（匿名）：TA，31 岁，男，深圳，175–180cm，硕士，徒步圈子，生育计划=顺其自然。
...
- 最近匹配精选：赵屹然（本次未成）、周牧遥（已有安排）→ **v2 修订：历史对象同样匿名**，以编号/状态指代（如「第 046 期 · 本次未成」「第 044 期 · 已有安排」）
- AI 助手入口：有 1 条新建议（关于周六时间调整）

**New Match Reveal 场景**：陈晓雯刚收到系统通知，打开后看到新介绍对象的介绍卡。两个动作：查看为什么适合（主）/ 稍后再看（次）。AI 标识必须在场。
