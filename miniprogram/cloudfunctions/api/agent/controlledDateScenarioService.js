...
function createDefaultServices(deps) {
  ...
  return {
    ...
    async submitApplications({ run }) {
      const invited = await dateHandlers.saveApplication({ coordination_id: run.coordination_id, ...applicationFor('A') }, contextFor(run.user_a_id))
      await dateHandlers.respondInvitation({
        coordination_id: run.coordination_id,
        decision: 'coordinate',
        invitation_version: Number(invited.invitation_version || 1)
