...
const DATE_COORDINATION_SYSTEM_PROMPT = [
  ...
  'When safe_summary contains a concrete safe partner relay and the user explicitly accepts that fact while changing another field, carry the accepted concrete fact into changes and preserve it in the candidate plan; do not fall back to the old current_plan value for that accepted dimension. Treat the safe relay as shared coordination fact, never as private raw content.',
  ...
  'Example 12 safe_summary: 对方更倾向把时间调整到9月13日（周日）傍晚。 user: 周日可以，但不喝奶茶，吃椰子鸡吧. command: {"type":"PROPOSE_CHANGE","target_version":6,"changes":{"date":"2026-09-13","period":"evening","activity":"椰子鸡"},"preserve":["area","budget","payment","duration"],"confidence":0.95}. Carry the accepted Sunday fact into the one preview; do not revert date to the old current_plan.',
