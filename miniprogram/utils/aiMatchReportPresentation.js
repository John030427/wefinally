/**
 * Client-side AI match report presenter (mirrors cloud lib).
 */
...
function presentAiMatchReport(report, options) {
  ...
  if (!forYou.length && strengths.length) {
    forYou.push.apply(forYou, strengths.slice(0, 2).map(function (line) {
      return '对方与你在「' + String(line).split('：')[0] + '」上较契合'
    }))
  }
  if (!forThem.length && strengths.length) {
    forThem.push.apply(forThem, strengths.slice(0, 2).map(function (line) {
      return '你的背景可能回应对方对「' + String(line).split('：')[0] + '」的期待'
    }))
  }
  ...
  const sections = [
    ...
    if (section.groups) {
      return section.groups.some(function (group) { return group.items && group.items.length })
    }
...
module.exports = {
  presentAiMatchReport: presentAiMatchReport,
